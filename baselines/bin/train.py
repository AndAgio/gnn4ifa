...
# from baselines.models import SVM, DecisionTree
...
class Trainer():
    def __init__(self,
        ...
        if chosen_model in ['svm', 'decision_tree']:
            self.mode = 'class'
    ...
    def get_dataset(self):
        ...
        self.datasetter = Datasetter(data_dir=self.download_dataset_folder,
                                     ...
                                     out_file=os.path.join(self.dataset_folder,
                                                           '{}_{}'.format(self.chosen_model, self.feat_set)))
        self.datasetter.run()
        self.train_set = self.datasetter.read_split(split='train')
        self.val_set = self.datasetter.read_split(split='val')
        self.test_set = self.datasetter.read_split(split='test')
